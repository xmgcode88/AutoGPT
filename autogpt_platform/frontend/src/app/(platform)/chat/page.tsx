...
import {
  CHAT_PAGE_TITLE_ZH,
  CHAT_SESSION_LABEL_ZH,
  NEW_CHAT_BUTTON_ZH,
  CHAT_LOADING_CREATING_SESSION_ZH,
  CHAT_LOADING_LOADING_ZH,
  CHAT_ERROR_CONTEXT_ZH,
} from "./i18n";
    ...
    <div className="flex h-full flex-col">
      ...
      <header className="border-b border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900">
        <div className="container mx-auto flex items-center justify-between">
          <h1 className="text-xl font-semibold">{CHAT_PAGE_TITLE_ZH}</h1>
          {sessionId && (
            <div className="flex items-center gap-4">
              <span className="text-sm text-zinc-600 dark:text-zinc-400">
                {CHAT_SESSION_LABEL_ZH}: {sessionId.slice(0, 8)}...
              </span>
              <button
      ...
      <main className="container mx-auto flex flex-1 flex-col overflow-hidden">
        ...
        {(isLoading || isCreating || (!sessionId && !error)) && (
          <ChatLoadingState
            message={isCreating ? CHAT_LOADING_CREATING_SESSION_ZH : CHAT_LOADING_LOADING_ZH}
          />
        )}

        {/* Error State */}
        {error && !isLoading && (
          <ChatErrorState error={error} onRetry={createSession} context={CHAT_ERROR_CONTEXT_ZH} />
        )}
